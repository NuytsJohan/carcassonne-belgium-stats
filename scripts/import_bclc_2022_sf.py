"""BCLC 2022 - Semi-finals (1/2 Finals) + placement semifinals for ranks 5-16.

Above black divider: winners' bracket 1/2 Finals (2 matches).
Below: three groups of 2 placement-bracket SFs separated by grey dividers:
  - QF losers -> ranks 5/6 vs 7/8 (stage '5th-8th SF')
  - LB R1 winners -> ranks 9/10 vs 11/12 (stage '9th-12th SF')
  - LB R1 losers -> ranks 13/14 vs 15/16 (stage '13th-16th SF')

Niels Ongena vs Nico Wellemans ended 95-95 in the image; final ranks (10 vs 11)
imply Niels advanced on tiebreak, so result='1'.
"""
from pathlib import Path

import duckdb

DB_PATH = Path(__file__).parents[1] / "data" / "carcassonne.duckdb"
TID = 28

MATCHES = [
    ("1/2 Finals",     1, "Nicolas Victor",           103, "Joren De Ridder",     68, "1"),
    ("1/2 Finals",     2, "Dominic R\u00e9b\u00e9rez", 96, "Raf Mesotten",       104, "2"),

    ("5th-8th SF",     1, "Els Smekens",               62, "Tim Onghena",         81, "2"),
    ("5th-8th SF",     2, "Celien Neven",              92, "Bert Jacobs",        107, "2"),

    ("9th-12th SF",    1, "Niels Ongena",              95, "Nico Wellemans",      95, "1"),
    ("9th-12th SF",    2, "Nele De Pooter",           113, "Bj\u00f6rn Petit",   103, "1"),

    ("13th-16th SF",   1, "Patrick De Wilde",          56, "Kim Peeters",         97, "2"),
    ("13th-16th SF",   2, "Geert Wouters",            101, "Philippe Kenens",    134, "2"),
]


def pid(conn, name_nl: str) -> int:
    row = conn.execute(
        "SELECT id FROM players WHERE LOWER(name_nl) = LOWER(?)", [name_nl]
    ).fetchone()
    assert row, f"Missing player: {name_nl}"
    return row[0]


def main():
    conn = duckdb.connect(str(DB_PATH))

    for stage in {m[0] for m in MATCHES}:
        c = conn.execute(
            "SELECT COUNT(*) FROM tournament_matches WHERE tournament_id=? AND stage=?",
            [TID, stage],
        ).fetchone()[0]
        if c:
            print(f"{stage} already has {c} rows for tid={TID}; aborting.")
            conn.close()
            return

    max_tm = conn.execute("SELECT COALESCE(MAX(id), 0) FROM tournament_matches").fetchone()[0]
    next_tm = max_tm + 1

    for stage, mnum, p1, s1, p2, s2, result in MATCHES:
        a_pid = pid(conn, p1)
        b_pid = pid(conn, p2)
        notes = "tiebreak (equal score)" if s1 == s2 else None
        gid = conn.execute(
            """
            INSERT INTO games (tournament_id, round, table_number, source, played_at)
            VALUES (?, ?, ?, ?, ?)
            RETURNING id
            """,
            [TID, 3, mnum, "manual", "2022-01-01"],
        ).fetchone()[0]

        a_rank = 1 if result == "1" else 2
        b_rank = 2 if result == "1" else 1
        conn.execute(
            "INSERT INTO game_players (game_id, player_id, score, rank) VALUES (?, ?, ?, ?)",
            [gid, a_pid, s1, a_rank],
        )
        conn.execute(
            "INSERT INTO game_players (game_id, player_id, score, rank) VALUES (?, ?, ?, ?)",
            [gid, b_pid, s2, b_rank],
        )
        conn.execute(
            """
            INSERT INTO tournament_matches
                (id, tournament_id, stage, match_number, player_1_id, player_2_id,
                 score_1, score_2, result, notes, game_id_1)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [next_tm, TID, stage, mnum, a_pid, b_pid, s1, s2, result, notes, gid],
        )
        next_tm += 1
        print(f"  [{stage} #{mnum}] {p1} {s1} - {s2} {p2}  result={result}  game_id={gid}")

    gcnt = conn.execute("SELECT COUNT(*) FROM games WHERE tournament_id=?", [TID]).fetchone()[0]
    tmcnt = conn.execute(
        "SELECT COUNT(*) FROM tournament_matches WHERE tournament_id=?", [TID]
    ).fetchone()[0]
    print(f"\nGames: {gcnt}, tournament_matches: {tmcnt}")

    conn.close()


if __name__ == "__main__":
    main()
